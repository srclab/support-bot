...
return [
    ...
    /*
     ...
     | except_operators_ids - список исключенных операторов на которых диалоги переводить нельзя
     |
     */
    'redirect_chats' => [
    ...
    /*
     ...
     |
     | enabled - Активность скрипта
     | job_queue - Название очереди используемой в работах сценария.
     | table_name - Название таблицы содержащей сценарии
     | answers_table_name - Название таблицы содержащей ответы пользователей на скрипт
     | exceptions_table_name - Название таблицы содержащей сообщения-исключения при которых скрипт запускать не нужно
     | select_message - Регулярное выражение условие для запуска скрипта
     | chat_closing_time - Время через которое чат будет закрыт при отсутствии ответа на сообщения скрипта
     | delay_before_running - Задержка перед запуском скрипта
     | enabled_for_user_ids - ид пользователей для которых скрипт активен
     | send_notification_period.period_begin - Время с которого начинать запуск скриптов для пользователей
     | send_notification_period.period_end - Время после которого не запускать скрипты для пользователей
     | clarification.message - Первое сообщение скрипта
     | clarification.final_step - ключ финального шага скрипта
     | clarification.steps - шаги скрипта
     | clarification.steps.id - ид варианта ответа скрипта ( должен быть уникальным )
     | clarification.steps.messages - сообщения отправляемые пользователю после выбора варианта ответа
     | clarification.steps.button - название кнопки варианта
     | clarification.steps.is_final - флаг что вариант является финальным и скрипт при его выборе прерывается
     | clarification.steps.for_operator - флаг что при выборе варианта необходимо перевести диалог на оператора
     | clarification.steps.messages - сообщения шага
     | clarification.steps.is_final - флаг что шаг является финальным и скрипт при этом шаге прерывается
     |
     */
    'scripts' => [
        'enabled' => env('SRCLAB_SUPPORT_BOT_SCRIPT_ENABLED', false),
        'job_queue' => env('SRCLAB_SUPPORT_BOT_SCRIPT_JOB_QUEUE', 'queue_5'),
        'table_name' => 'support_bot_pending_scripts',
        'answers_table_name' => 'support_bot_pending_scripts_answers',
