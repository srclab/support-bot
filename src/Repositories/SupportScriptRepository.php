...
class SupportScriptRepository extends Repository
{
     ...
     */
    public function getNextSendingScriptUser()
    {
        return $this->query()
            ->where('send_first_message_at', '<', Carbon::now())
            ->where('step', 0)
            ->first();
    }
     ...
     *
     * @param int $client_id
     * @param string|Carbon $send_first_message_at
     * @param int $step
     */
    public function addRecord($client_id, $send_first_message_at, $step = 0)
    {
        $this->query()
            ->insert(compact('client_id', 'step', 'send_first_message_at'));
    }
